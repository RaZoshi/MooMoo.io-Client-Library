...
// @grant       none
// ==/UserScript==
//make knockmath, rework ass hating system
var tick = 0;
let fzStatusMenu = document.createElement("div");
fzStatusMenu.id = "fz's op status menu", fzStatusMenu.style.position = "absolute", fzStatusMenu.style.color = "white",
...
document.body.prepend(modMenu),modMenu.style.position = "absolute",modMenu.style.textAlign = "left",
    modMenu.style.display = "none",modMenu.style.width = "300px",modMenu.style.height = "240px", modMenu.style.top = "4%",modMenu.style.left = "1%",
    modMenu.style.backgroundColor = "rgb(0, 0, 0, 0.3)",modMenu.style.color = "white",modMenu.style.borderTopLeftRadius="25px",//, modMenu.style.overflowY = "scroll",
    modMenu.style.borderTopRightRadius="25px",modMenu.style.borderBottomRightRadius="25px", modMenu.style.borderBottomLeftRadius="25px";
modMenu.innerHTML = `
<style>
.indent {
margin-left: 10px;
...
<br>
To connect bots, press "G". Option toggles sometimes may need the bot to get killed before functioning correctly.
</div>
...
const wait = async ms => new Promise(action => setTimeout(action, ms)), connectBot = (code, thebot) => {
    let ws = new WebSocket(websocket.url.split("&")[0] + "&token=" + encodeURIComponent(code));
    ...
    ws.onmessage = message => {
        ...
        if(data[0] == "p" && document.getElementById("syncteam").checked && ws.weapons[1]) {
            ws.autoaiming = true;
            setTimeout(() => {
                ws.autoaiming = false;
            }, 250);
            ...
            for(let i = 0; i < data[1].length / 13; i++) {
                ...
                if(playerInfo[0] == ws.id) {
                    ...
                    if(Date.now() - ws.justShot <= 2000) {
                        ...
                        ws.emit(["13c", [0, 6, 0]]);
                        ws.emit(["33", [null]]);
                    }else if(document.getElementById("botConfig").value == 2 && (ws.weapons[1] == 13 || ws.weapons[1] == 15) && ws.autoaiming == false) {
                        if(Math.sqrt(Math.pow((myPlayer.y - ws.posy), 2) + Math.pow((myPlayer.x - ws.posx), 2)) > 200) {
                            ws.emit(["33", [Math.atan2(myPlayer.y - ws.posy, myPlayer.x - ws.posx)]]);
                            ws.emit(["5", [ws.weapons[0], true]]);
                            ws.emit(["c", [1, Number.MAX_VALUE]]);
                            ws.emit(["c", [0, Number.MAX_VALUE]]);
                        }else {
                            if(Math.sqrt(Math.pow((myPlayer.y - ws.posy), 2) + Math.pow((myPlayer.x - ws.posx), 2)) > 100) {
                                ws.emit(["33", [Math.atan2(myPlayer.y - ws.posy, myPlayer.x - ws.posx)]]);
                            }else {
                                ws.emit(["33", [null]]);
                            }
                            if(nearestEnemy.length) {
                                ws.emit(["5", [ws.weapons[1], true]]);
                                ws.emit(["c", [1, Math.atan2(nearestEnemy[2] - ws.posy, nearestEnemy[1] - ws.posx)]]);
                                ws.emit(["c", [0, Math.atan2(nearestEnemy[2] - ws.posy, nearestEnemy[1] - ws.posx)]]);
                                ws.emit(["2", [Math.atan2(nearestEnemy[2] - ws.posy, nearestEnemy[1] - ws.posx)]]);
                            }
                        }
                        ws.emit(["13c", [0, 11, 1]]);
                        ws.emit(["13c", [0, 20, 0]]);
                    }else if(document.getElementById("botConfig").value == 1 && (ws.weapons[1] == 13 || ws.weapons[1] == 15) && ws.autoaiming == false) {
                        if(Math.sqrt(Math.pow((myPlayer.y - ws.posy), 2) + Math.pow((myPlayer.x - ws.posx), 2)) > 100) {
                            ws.emit(["33", [Math.atan2(myPlayer.y - ws.posy, myPlayer.x - ws.posx)]]);
                            ws.emit(["5", [ws.weapons[0], true]]);
                            ws.emit(["c", [1, Number.MAX_VALUE]]);
                        ...
                        let angles = [-0.77, -2.34, 2.35, 0.77, 1.57, 3.14, -1.57, 0];
                        if(ws.weapons[0] != 4) {
                            ws.place(ws.items[3], ws.moveRan.angle - (225 / 180 * Math.PI));
                            ws.place(ws.items[3], ws.moveRan.angle + (225 / 180 * Math.PI));
                        ...
                        ws.emit(["13c", [0, 6, 0]]);
                        ws.upgrade();
                        if((Date.now() - ws.moveRan.lastChange >= 10000) || (Math.sqrt(Math.pow((ws.moveRan.y - ws.posy), 2) + Math.pow((ws.moveRan.x - ws.posx), 2)) > 3300)) {
                            ws.moveRan.angle = angles[Math.floor(Math.random() * angles.length)];
                ...
                }else if(!(playerInfo[0] == ws.id || playerInfo[7] && playerInfo[7] == ws.clan)) {
...
, function(e, t, n) {
    ...
    function $t(e, t) {
        ...
        for (var n = 0; n < l.list.length; ++n) {
            ...
            if(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) {
                document.getElementById("actionBarItem" + i).style.display = [0, 3, 6, 10].indexOf(l.list[n].id) >= 0 ? "inline-block" : "none";
            }else {
    ...
    function pn() {
        ...
        }else if((document.getElementById("leftConfig").value == 0 && tankSpam == true) || document.getElementById("autogrind").checked == true) {
            _360HitsVaribleVisual = true;
            return 90**100
        }else if(document.getElementById("leftConfig").value == 1 && tankSpam == true && nearestEnemy.length) {
            return nearestEnemyAngle
        }else if(modStatus == "auto bull spam" || (modStatus == "oneticking" && R.weapons[0] != 4)) {
            _360HitsVaribleVisual = false;
    ...
    var audios = [
        ...
        new Audio("https://cdn.discordapp.com/attachments/967213871267971072/1027421527722963044/Egzod_Maestro_Chives__Alaina_Cross_-_No_Rival_Official_Lyric_Video.mp3"),
    ];
    function sendChat(e) {
        let chats = [];
        audios[e].play();
    ...
    var keys = [], mills = {w: false, a: false, s: false, d: false, y: 0, x: 0, aim: 0, status: false, space: false}, spamchat = false, chatQueue = [], endChatQueue = null;
    ...
    window.addEventListener("keydown", s.checkTrusted((function(e) {
        ...
        84 == t && autoaim == false && "chatbox" !== document.activeElement.id.toLowerCase() && (onetick = true);
        87 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.w = true);
        65 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.a = true);
        83 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.s = true);
        68 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.d = true);
        32 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.space = true);
        if(e.key == "G" && "chatbox" !== document.activeElement.id.toLowerCase()) {
            let promises = [];
            for(let i = 0; i < 3; i++) {
                promises.push(grecaptcha.execute(("6LevKusUAAAAAAFknhlV8sPtXAk5Z5dGP5T2FYIZ"), { action : 'homepage' }));
            }
        ...
        if(e.keyCode == 190 && document.getElementById("syncteam").checked && autoaim == false) {
            if(nearestEnemy.length && R.team && "chatbox" !== document.activeElement.id.toLowerCase()) {
                r.send("14", 1);
            }
        }
        if(e.key && e.key == "C" && "chatbox" !== document.activeElement.id.toLowerCase()) {
            if(spamchat == false) {
    ...
    window.addEventListener("keyup", s.checkTrusted((function(e) {
        if (R && R.alive) {
            var t = e.which || e.keyCode || 0;
            keys[t] = false;
            84 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (onetick = false, r.send("33", null));
            87 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.w = false);
            65 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.a = false);
            83 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.s = false);
            68 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (mills.d = false);
    ...
    function bn() {
        ...
        ((null == wn || null == e || Math.abs(e - wn) > .3) && lockMove == false) && (r.send("33", e),
        wn = e)
    }
    var insta = false;
    function xn() {
    ...
    function Mn(e, t, n, i) {
        m.showText(e, t, 50, .18, document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1 ? 2000 : 500, Math.abs(n), n >= 0 ? "#fff" : "#8ecc51")
    }
    var An = 99999;
    function howdie() {
        if(R && R.skinIndex == 45) {
    ...
    function Cn(e) {
        ...
        if(document.getElementById("autogrind").checked == false) {
            if(b && Math.hypot(b.y - R.y2, b.x - R.x2) < 300 && document.getElementById("autoreplace").checked == true) {
                if(nearestEnemy.length && dist(nearestEnemy, R) < 200) {
                    ...
                    if(dist(nearestEnemy, R) < 160 && autoaim == false && R.primary.reload == 1) {
                        autoaim = true;
                        Jt(7, 0);
                    ...
                    for(let i = nearestEnemyAngle-Math.PI/3; i < nearestEnemyAngle+Math.PI/3; i+=Math.PI/18) {
                        place(R.items[2], i);
                        place(R.items[2], -i);
                    }
                }
    ...
    function getFZdir() {
        ...
        }else if(R.primary.reload == 1 && (modStatus == "auto bull spam" || modStatus == "oneticking")) {
            return nearestEnemyAngle
        }else {
            return Math.atan2(ie - z / 2, ne - F / 2);
        }
    ...
    function Xn(e, t, n) {
        ...
        for(var i = 0; i < W.length; i++) {
            ...
            if(_.zIndex == n) {
                ...
                if(_.visible) {
                    _.skinRot += .002 * P;
                    let aemode = document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0 ? true : false;
                    let fzmode = document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1 ? true : false;
                    L = (_ == R && fzmode == true ? getFZdir() : _ == R && !document.getElementById("visuals").checked && aemode == false ? pn() > 1e100 ? Math.atan2(ie - z / 2, ne - F / 2) : pn() : _.dir) + _.dirPlus;
                    be.save();
                    be.translate(_.x - e, _.y - t);
                    be.rotate(L);
                    if(_ == R) {
                        let weapon = (R.weapons[1] == 10 ? 10 : R.weapons[0]);
                        if(fzmode == true && (weapon == 10 ? R.secondary.reload == 1 : R.primary.reload == 1) &&
                           (intrap == true || (tankSpam == true && document.getElementById("leftConfig").value == 0)) &&
                          _360HitsVaribleVisual == true) {
                        }else {
                            Nn(_, be);
                        }
    ...
    function Nn(e, t) {
        ...
        if (e.tailIndex > 0 && function(e, t, n) {
            if (!(Gn = Qn[e + (document.getElementById("textpack").checked ? "lol" : 0)])) {
                var i = new Image;
                i.onload = function() {
                    this.isLoaded = !0,
                    this.onload = null
                ...
                Qn[e + (document.getElementById("textpack").checked ? "lol" : 0)] = i,
                Gn = i
            }
            var r = $n[e];
            if (!r) {
        ...
        function e(t, n, i, r) {
            if (!(Gn = Jn[t + (document.getElementById("textpack").checked ? "lol" : 0)])) {
                var s = new Image;
                s.onload = function() {
                    this.isLoaded = !0,
                    this.onload = null
                ...
                Jn[t + (document.getElementById("textpack").checked ? "lol" : 0)] = s,
                Gn = s
            }
            var a = i || Kn[t];
            if (!a) {
    ...
    function ri(e, t) {
        ...
        if (!n || t) {
            ...
            t || (r.fillStyle = (R && e.owner && e.owner.sid == R.sid ? "" : (e.owner && R && R.team && isAlly(e.owner.sid) ? "" : "#780c0c"))),
            ((e.name.includes("spike") && r.fill() || e.name.includes("pit trap")) && r.fill())
            n = i,
    ...
    function handleBullets(sid) {
        if(R.sid != sid && !isAlly(sid)) {
            let index = Ii(sid);
            if(dist2(index, R) < 250 && (index.primary.id == 4 || index.primary.id == 5) && index.primary.variant > 1 && index.primary.reload == 1) {
    ...
    function fi(e, t, n, i, r, s, a, o) {
        lt && (J.addProjectile(e, t, n, i, r, s, null, null, a).sid = o, function(e, t, n, i, r) {
            let min = 1 / 0;
            let id = {
                secondary: {reload: 1},
                turret: 0,
            ...
            for (let i = 0; i < W.length; i++) {
                ...
                (_ = W[i]) && _.visible && user && l.weapons[user].projectile !== undefined && l.projectiles[l.weapons[user].projectile].speed == r &&
            ...
            if (Math.sqrt(min) > 60) {
                if (r == 1.5) {
                    for (let i = 0; i < W.length; i++) {
                        (_ = W[i]) && _.visible && min > (_.x2 * 1.5 - _.x1 / 2 - e + Math.cos(n) * 10) ** 2 + (_.y2 * 1.5 - _.y1 / 2 - t + Math.sin(n) * 10) ** 2 && (
                            id = _,
    ...
    function findSource(dmg) {
        ...
        for(let i = 0; i < inRange.length; i++) {
            if(dmg == sr(l.weapons[inRange[i].primary.id].dmg * variantMulti(inRange[i].primary.variant) * 1.5)) {
                return [inRange[i].sid, inRange[i].primary.id];
            }else if(dmg == sr(l.weapons[inRange[i].primary.id].dmg * variantMulti(inRange[i].primary.variant))) {
                return [inRange[i].sid, inRange[i].primary.id];
            }
            if(inRange[i].secondary.id == 10) {
                if(dmg == sr(l.weapons[inRange[i].secondary.id].dmg * variantMulti(inRange[i].secondary.variant) * 1.5)) {
                    return [inRange[i].sid, inRange[i].secondary.id];
                }else if(dmg == sr(l.weapons[inRange[i].secondary.id].dmg * variantMulti(inRange[i].secondary.variant)) + sr(25)) {
                    return [inRange[i].sid, inRange[i].secondary.id];
                }else if(dmg == sr(l.weapons[inRange[i].secondary.id].dmg * variantMulti(inRange[i].secondary.variant))) {
                    return [inRange[i].sid, inRange[i].secondary.id];
                }
            }else {
    ...
    function potDmg(source) {
        ...
        for(let i = 0; i < inRange.length; i++) {
            if(inRange[i].sid == source[0]) {
                if(source[1] < 9) {
                    if(inRange[i].secondary.reload + (111/l.weapons[inRange[i].secondary.id || 15].speed) >= 1) {
                        potDmg += inRange[i].secondary.dmg * variantMulti(inRange[i].secondary.id) + (inRange[i].primary.variant == 3 ? 5 : 0) + (inRange[i].secondary.variant == 3 ? 5 : 0);
                    ...
                    if(inRange[i].primary.reload + (111/l.weapons[inRange[i].primary.id].speed) >= 1) {
                        potDmg += l.weapons[inRange[i].primary.id].dmg * variantMulti(inRange[i].primary.id) * 1.5 + (inRange[i].primary.variant == 3 ? 5 : 0) + (inRange[i].secondary.variant == 3 ? 5 : 0);
                        if(inRange[i].turret == 1) {
                            potDmg += 25;
                        }
                    }
                ...
                if(inRange[i].primary.reload == 1) {
                    priDmg += l.weapons[inRange[i].primary.id].dmg * variantMulti(inRange[i].primary.id) * 1.5 + (inRange[i].primary.variant == 3 ? 5 : 0) + (inRange[i].secondary.variant == 3 ? 5 : 0);
                    if(inRange[i].turret == 1) {
                        priDmg += 25;
                    }
                }
                if(inRange[i].secondary.reload == 1) {
                    secDmg += inRange[i].secondary.dmg + (inRange[i].primary.variant == 3 ? 5 : 0);
                    if(inRange[i].turret == 1) {
                        secDmg += 25;
                    }
                }
    ...
    function healing(d) {
        ...
        if(inRange.length && (document.getElementById("lowDmgHeal").checked ? d >= 8 : true)) {
            if(dmg - pot[0] <= 0) {
                if(pot[1] && ((dmg - pot[0] + 25) > 0) && (otSoldier == false && onlySoldier == false)) {
                    if(document.getElementById("stressHeal").checked && inRange.length > 1) {
                        (document.getElementById("tickyheal").checked ? addEvent : setTimeout)(() => {
                            heal();
                        }, (document.getElementById("tickyheal").checked ? 1 : 50));
                    }else {
                        empAnti();
                    }
        ...
        if((d == 60 || d == 68) && canAB == true && autoaim == false && R.primary.reload == 1) {
            canAB = false;
            autoaim = true;
            Jt(7);
    ...
    setInterval(() => {
        ...
        if(R && isAlive == true && "chatbox" !== document.activeElement.id.toLowerCase()) {
        ...
        let nearest = N.filter(e => e.active && e.trap).sort((a, b) => Math.hypot(a.y-R.y2, a.x-R.x2) - Math.hypot(b.y-R.y2, b.x-R.x2))[0];
        if(nearest && nearest.trap && Math.hypot(nearest.y - R.y2, nearest.x - R.x2) < 80 && nearest.owner.sid != R.sid && !isAlly(nearest.owner.sid) && nearest.active) {
            intrap = true;
        ...
        if(intrap == true && mills.space == false && autoaim == false) {
            ...
            let spikes = N.filter(e => e.dmg && e.sid != R.sid && !isAlly(e.sid) && Math.hypot(e.y - R.y, e.x - R.x) < 130 && e.active);
            let buildings = N.filter(e => !e.ignoreCollision && !e.dmg && Math.hypot(e.y - R.y, e.x - R.x)/1.56 < l.weapons[weapon].range && e.active && e.health);
    ...
    var trapinfo = {last: false, health: 700, location: {x: 0, y: 0}}, intrap = false, otSoldier = false;
    function doinsta(e) {
        insta = false;
        if(e < 3 && document.getElementById("doinstachat").checked && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0)) sn(document.getElementById("instachat").value);
        if(e == 3) {
            onetick = false;
            lockMove = true;
    ...
    function autopush(spikes) {
        let spike = spikes.sort((a, b) => dist(nearestEnemy, a) - dist(nearestEnemy, b))[0];
        let enemyToSpike = Math.atan2(spike.y - nearestEnemy[2], spike.x - nearestEnemy[1]);
        let distance = Math.hypot(spike.x - nearestEnemy[1], spike.y - nearestEnemy[2]) + 30;
        let position = {
    ...
    function Ti(e) {
        ...
        if(mills.status) {
            mills.w || !mills.a || mills.s || mills.d ?
                ...
                mills.w && mills.a && !mills.s && !mills.d &&
                (mills.aim = 0.77) : mills.aim = 1.57 : mills.aim = 3.14 : mills.aim = -1.57 : mills.aim = 0;}
        if(Math.sqrt(Math.pow((mills.y - R.y), 2) + Math.pow((mills.x - R.x), 2)) > 100) {
            if(mills.status) {
        ...
        let trap;
        let myTraps = N.filter(e => e.owner && (e.owner.sid == R.sid || isAlly(R.sid)) && e.trap);
        if (myTraps.length && nearestEnemy.length) {
            for (let item of myTraps) {
                if(Math.hypot(item.y - nearestEnemy[2], item.x - nearestEnemy[1]) <= 45){
                    trap = item;
                }
            }
        }
        if(R.skins[11] && R.tails[21] && nearestEnemy.length) {
            ...
            preHit.info = [];
            for(let i = 0; i < inRange.length; i++) {
                if(trap && inRange[i].secondary.id == 10 && document.getElementById("otsoldierpre").checked &&
                   inRange[i].secondary.reload + (111/l.weapons[inRange[i].secondary.id].speed) >= 1
                   &&
                   inRange[i].secondary.reload + (111/l.weapons[inRange[i].secondary.id].speed) <= 1.2
                  ) {
                    preHit.info.push(inRange[i]);
                }else if(
                    inRange[i].primary.reload + (111/l.weapons[inRange[i].primary.id].speed) >= 1
                    &&
                    Math.round((inRange[i].primary.reload + (111/l.weapons[inRange[i].primary.id].speed))*100)/100 <= (inRange[i].primary.id == 5 ? 1.15 : 1.2)
                ) {
                    preHit.info.push(inRange[i]);
                }
            }
            if(preHit.info.length && document.getElementById("otsoldierpre").checked) {
                canTickBoi = true;
            }else {
                canTickBoi = false;
            }
        ...
        if(nearestEnemy.length && autoaim == false && R.items[4] == 15 && document.getElementById("autogrind").checked == false && document.getElementById("autoreplace").checked == true) {
            autoplace();
        }
        if(tickQueue[tick]) {
        ...
        if(nearestEnemy.length && insta == true && R.primary.reload == 1 && R.secondary.reload == 1 && R.turret == 1 && R.weapons[1]) {
            let cando = N.some(e => dist2(e, R) < dist(nearestEnemy, R) && (Math.abs(Math.atan2(e.y-R.y, e.x-R.x) - nearestEnemyAngle) <= e.scale/dist2(e, R)) && e.active && !e.ignoreCollision);
            if(dist(nearestEnemy, R)/1.65 < l.weapons[R.weapons[0]].range && R.weapons[1] != 10 && !cando) {
                if((R.bullTick && R.shameCount > 3 && (((tick - R.bullTick) % 9) == 8) && document.getElementById("shamerev").checked) && (nearestEnemy[9] == 11 && R.primary.variant == 0)) {
                    doinsta(2);
                }else if(nearestEnemy[9] != 6 && nearestEnemy[9] != 22 && R.skins[53]) {
                    doinsta(1);
                }else {
                    doinsta(0);
                }
        ...
        let candotick = R.weapons[0] == 4 ? nearestEnemy.length && ((tick - Ii(nearestEnemy[0]).bullTick) % 9 == 8) : true;
        if(autoaim == true) {
            modStatus = "instaing";
        }else if(intrap == true && mills.space == false) {
            modStatus = "autobreaking";
        }else if((tankSpam == true && mills.space == false) || document.getElementById("autogrind").checked == true) {
            if(document.getElementById("autogrind").checked == false) {
                if(document.getElementById("leftConfig").value == 0) {
                    ...
                    r.send("2", 90**100);
                }else if(document.getElementById("leftConfig").value == 1 && nearestEnemy.length) {
                    modStatus = "bowspam";
                    Sn(R.weapons[1], true);
                    r.send("c", 1, nearestEnemyAngle);
                    r.send("c", 0, nearestEnemyAngle);
            ...
            if(distance > 235 && distance < (R.weapons[0] == 4 ? 250 : 240)) {
                r.send("33", null);
                biomehat();
                if(R.primary.reload == 1 && R.primary.reload == 1 && R.turret == 1 && autoaim == false && (((nearestEnemy[9] == 22 || nearestEnemy[9] == 6) && canTickBoi == true) || (nearestEnemy[9] != 22 && nearestEnemy[9] != 6)) && candotick) {
                    doinsta(3);
                }
            }else {
                ...
                if(distance > (R.weapons[0] == 4 ? 190 : 185) && distance < (R.weapons[0] == 4 ? 300 : 290)) {
                    Jt(0, 1);
                }
                if(distance > (R.weapons[0] == 4 ? 215 : 210) && distance < (R.weapons[0] == 4 ? 275 : 265)) {
                    if(distance > (R.weapons[0] == 4 ? 230 : 225) && distance < (R.weapons[0] == 4 ? 260 : 250)) {
                        Jt(40, 0);
                    }else{
                        Jt(22, 0);
                    }
        ...
        }else if((lastNearestDist > 180 && nearestDist < 180 || preHit.amount.length == 1) && nearestEnemy.length && Ii(nearestEnemy[0]).primary.variant == 0) {
            modStatus = "pab";
            canAB = true;
            Jt(11);
            Jt(21, 1);
        }else if(
            (insta == false && nearestEnemy.length && dist(nearestEnemy, R)/1.65 < l.weapons[R.weapons[0]].range && R.secondary.reload == 1 && document.getElementById("autoBSpam").checked)
            ||
            (mills.space == true)
        ) {
            ...
            if(mills.space == true && trap) {
                let spikes = N.filter(e => e.dmg && (e.owner.sid == R.sid || isAlly(e.owner.sid)) && dist2(e, trap) <= 130);
    ...
    function doAutoUpgrade() {
        if(document.getElementById("autoupgrade").checked == true && isAlive == true) {
            if(document.getElementById("upgradeType").value == 0) {
                if(R.items[5] != l.list.findIndex(e => e.name.includes(document.getElementById("sixbuilding").value))) {
                    r.send("6", 7);
                    r.send("6", l.list.findIndex(e => e.name.includes("cookie"))+16);
                    r.send("6", l.list.findIndex(e => e.name.includes("pit"))+16);
                    r.send("6", l.list.findIndex(e => e.name.includes("greater"))+16);
                    r.send("6", 10);
                    r.send("6", l.list.findIndex(e => e.name.includes(document.getElementById("sixbuilding").value))+16);
                }
            }else {
                if(R.items[5] != l.list.findIndex(e => e.name.includes(document.getElementById("sixbuilding").value))) {
                    r.send("6", 5);
    ...
    function canTick(e) {
        ...
        if(R.shameCount > 5 && (document.getElementById("bullticktype").value == 1 || document.getElementById("bullticktype").value == 2)) {
            if(((tick - R.bullTick) % 9 == 0) || (needTick > 1)) {
                return true;
            }
        }else {
            if(document.getElementById("bullticktype").value == 0 || document.getElementById("bullticktype").value == 1) {
                if(enemy && nearestEnemy.length && dist(nearestEnemy, R)/1.56 > (l.weapons[enemy.primary.id].range - (document.getElementById("bullticktype").value == 1 ? l.weapons[enemy.primary.id].range/2 : 0))) {
    ...
    function bullSpamEndHats() {
        if(turrets.length && R.skins[22]) {
            ...
            Jt(6);
            Jt(13, 1);
        }else if(nearestEnemy.length && preHit.amount.length && Ii(nearestEnemy[0]).primary.variant == 0) {
            canAB = true;
            Jt(11);
            Jt(21, 1);
    ...
    function dist(a, b, nexttick){
        if(nexttick) {
            let c = Ii(a[0]);
            return Math.sqrt(Math.pow((b.pre.y || b.y2) - (c.pre.y || c.y2), 2) + Math.pow((b.pre.x || b.x2) - (c.pre.x || c.x2), 2))
        }else {
    ...
    function dist2(a, b, nexttick){
        if(nexttick) {
            return Math.sqrt(Math.pow((b.pre.y || b.y2) - (a.pre.y || a.y2), 2) + Math.pow((b.pre.x || b.x2) - (a.pre.x || a.x2), 2))
        }else {
    ...
    function e() {
        ...
        function() {
            ...
            be.lineWidth = 4,
            be.strokeStyle = "#000",
            be.globalAlpha = document.getElementById("visuals").checked && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1) ? .0 : .06,
            be.beginPath();
            let thefz = document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1;
            for (var p = thefz ? ((14400-f) % 1440) : -U; p < oe; p += thefz ? 1440 : ce / 18)
                p > 0 && (be.moveTo(p, 0),
                be.lineTo(p, ce));
            for (var g = thefz ? ((14400-d) % 1440) : -D; g < ce; g += thefz ? 1440 : ce / 18)
                p > 0 && (be.moveTo(0, g),
                be.lineTo(oe, g));
            for (be.stroke(),
            be.globalAlpha = 1,
            ...
            for(let i = 0; i < N.length; i++) {
                let _ = N[i];
                _.currentHealth > 0 && _.active && _.currentHealth < _.health && document.getElementById("buildHP").checked && (
                    o.healthBarWidth,
                    be.globalAlpha = 1,
                    be.fillStyle = rt,
            ...
            a = 0; a < W.length + Y.length; ++a)
                if ((_ = W[a] || Y[a - W.length]).visible && (10 != _.skinIndex || _ == R || _.team && _.team == R.team)) {
                    let fzmode = document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1 ? true : false;
                    var w = (_.team ? "[" + _.team + "] " : "") + (_.isPlayer && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) && (fzmode ? true : document.getElementById("visuals").checked) ? fzmode == true && _ == R ? "" : "["+_.primary.id+(_.secondary.id ? "/" + _.secondary.id : "")+"] " : "") + (_.name || "");
                    if ("" != w) {
                        ...
                        Rn.crosshair.isLoaded && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) &&
                            insta && nearestEnemy.length && _.sid == nearestEnemy[0] && _.isPlayer &&
                    ...
                    R && _ == R && (fzmode == true ? true : document.getElementById("visuals").checked) && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) && (
                        be.textAlign = "center",
                        be.fillStyle = "#fff",
                        be.lineJoin = "round",
                        ...
                        be.lineWidth = 6,
                        be.strokeText("["+needTick+","+insta+","+inRange.length+"]",_.x - f,_.y - d + _.scale + o.nameY+(30)),
                        be.fillText("["+needTick+","+insta+","+inRange.length+"]",_.x - f,_.y - d + _.scale + o.nameY+(30))
                    )
                    if(_.isPlayer && (fzmode ? true : document.getElementById("visuals").checked) && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0)) {
                        let placeTriX = R.x + Math.cos(Math.atan2(_.y - R.y, _.x - R.x)) * (fzmode ? dist2(_, R)/3 : 100);
                        let placeTriY = R.y + Math.sin(Math.atan2(_.y - R.y, _.x - R.x)) * (fzmode ? dist2(_, R)/3 : 100);
                        let tmpBE = be;
                        if(_ != R && (_.team != R.team || !_.team)) {
                            tmpBE.save();
                            tmpBE.translate(placeTriX - f, placeTriY - d);
                            tmpBE.rotate(Math.atan2(_.y - R.y, _.x - R.x));
                            tmpBE.fillStyle = fzmode ? "#fff" : "#000";
                            ai(tmpBE, 1.5, fzmode ? 15 : 20, fzmode ? 15 : 20, 1);
                            tmpBE.fill();
                            tmpBE.restore();
                        }else if(_ != R && _.team && _.team == R.team && !fzmode) {
                            tmpBE.save();
                            tmpBE.translate(placeTriX - f, placeTriY - d);
                            tmpBE.rotate(Math.atan2(_.y - R.y, _.x - R.x));
                            tmpBE.fillStyle = "#fff";
                    ...
                    _.isPlayer && (fzmode == true ? true : document.getElementById("visuals").checked) && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) && (
                        ...
                        be.lineWidth = _.nameScale ? 11 : 8,
                        be.lineJoin = "round",
                        be.strokeText(_.shameCount, _.x - f + be.measureText(w).width / 2 + o.crownPad, _.y - d - _.scale - o.nameY),
                        be.fillText(_.shameCount, _.x - f + be.measureText(w).width / 2 + o.crownPad, _.y - d - _.scale - o.nameY)
                    )
                    _.isPlayer && (fzmode == true ? true : document.getElementById("visuals").checked) && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) && (
                        o.reloadBarWidth,
                        be.fillStyle = rt,
                        ...
                        be.fill(),
                        be.fillStyle = "#fff000",
                        be.roundRect(_.x - f - 50, _.y - d + _.scale + o.nameY - 13 + o.healthBarPad, 2 * 23.5 * (_.primary.reload), 16 - 2 * o.healthBarPad, 10),
                        be.fill(),
                        o.reloadBarWidth,
                        be.fillStyle = rt,
            ...
            !function(e) {
                if (R && R.alive) {
                    ...
                    for(let i = 0; i < botInfo.length; i++) {
                        if(botInfo[i]) {
                            Ke.fillStyle = "#ffa500";
                            si(botInfo[i][1] / o.mapScale * Ne.width, botInfo[i][2] / o.mapScale * Ne.height, 7, Ke, !0);
                        }
                    }
                    for(let i = 0; i < botEnemies.length; i++) {
                        if(botEnemies[i] && botEnemies[i][0] != R.sid && !isAlly(botEnemies[i][0]) && !botInfo.find(e => e[0] == botEnemies[i][0])) {
                            Ke.fillStyle = "#ff0000";
                            si(botEnemies[i][1] / o.mapScale * Ne.width, botEnemies[i][2] / o.mapScale * Ne.height, 7, Ke, !0);
                        }
                    }
                    let check = (s) => {
                    ...
                    for(let i = 0; i < breakTrack.length; i++) {
                        if(check(breakTrack[i]) == true && document.getElementById("breakTracker").checked) {
                            Ke.fillStyle = "#000";
                            Ke.font = "34px Hammersmith One";
                            Ke.textBaseline = "middle";
                            Ke.textAlign = "center";
                            Ke.fillText("L", breakTrack[i].x / o.mapScale * Ne.width, breakTrack[i].y / o.mapScale * Ne.height);
                        }
                    }
                }
            }(P),
        ...
        let fzmode = document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1 ? true : false;
        if(oe && ce && document.getElementById("darkmode").checked && fzmode == false && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0)) {
...
, function(e, t) {
    e.exports.AnimText = function() {
        ...
        this.render = function(e, t, n) {
            if(document.getElementById("visuals").checked && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 0) && !(document.getElementById("doExternalVisuals").checked == true && document.getElementById("visualType").value == 1)) {
...
, function(e, t, n) {
    ...
    e.exports = function(e, t, n, l, h, u, f, d, p, g, m, y, k, v) {
        ...
        this.spawn = function(e) {
            ...
            this.weaponXP = [],
            this.reloads = {},
            this.primary = {reload: 1, id: 0, variant: 0, reloadid: 0, dmg: 25},
            this.secondary = {reload: 1, id: undefined, variant: undefined, reloadid: undefined, dmg: 0},
            this.turret = 1,
            this.pre = {x: 0, y: 0},
            this.last = {x: 0, y: 0, tick: 0},
            this.bullTick = 0;
        }
        ,
            ...
            if (this.alive) {
                if(this.weaponIndex < 9) {
                    if(this.primary.id == this.weaponIndex) {
                        ...
                        this.primary.dmg = window.weapons[this.weaponIndex].dmg;
                    }else {
                    ...
                    if(this.secondary.id == this.weaponIndex) {
                        ...
                        this.secondary.dmg = window.weapons[this.weaponIndex].dmg || window.secondaryDmg(this.weaponIndex);
                    }else {
                ...
                if(this.buildIndex == -1) {
                    if(this.weaponIndex < 9) {
                        if(this.primary.reloadid == this.weaponIndex) {
                            this.primary.reload = Math.min(this.primary.reload + 111/window.weapons[this.weaponIndex].speed, 1);
                        }else {
                        ...
                        if(this.secondary.reloadid == this.weaponIndex) {
                            this.secondary.variant = this.weaponVariant;
                            this.secondary.reload = Math.min(this.secondary.reload + 111/window.weapons[this.weaponIndex].speed, 1);
                        }else {
                ...
                this.turret = Math.min(this.turret + 0.0444444444, 1);
            }
        }
        ,
