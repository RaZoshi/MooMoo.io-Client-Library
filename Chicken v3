...
, function(e, t, n) {
    ...
    Featured Person:
    <div class="spanLink" id="featuredYoutube"></div>
    `;
    ...
    function On() {
        ...
        if(R.kills > Ue.innerText) {
            if(toggles.killChat()) {
                r.send("ch", "Dumbasses down: " + R.kills);
                setTimeout(() => {
                    r.send("ch", "I'm Super Pro");
                }, 750);
            }
        }
    ...
    function Ti(e) {
        ...
        if(Math.sqrt(Math.pow((mills.y - R.y), 2) + Math.pow((mills.x - R.x), 2)) > 99) {
            if(mills.status && (R.itemCounts[3] < 99 || !R.itemCounts[3])) {
                place(R.items[3], mills.aim + toRad(180));
                place(R.items[3], mills.aim - toRad(69 + 180));
                place(R.items[3], mills.aim + toRad(69 + 180));
            }else if(!(R.itemCounts[3] < 99 || !R.itemCounts[3])) {
                mills.status = false;
            }
            mills.x = R.x;
            mills.y = R.y;
